...
def user_mgmt( ):
    while True :
        ...
        if u==3:
            break
def create_database():
    mydb=mysql.connector.connect(host="localhost",user="root",database="stock",password="bhawna21")
...
def list_order():
        ...
        print("\t\t\t\t\t\t\t ORDER DETAILS")
        print("-" * 85)
        print("orderid Date Product code price quantity Suppliern Category")
        print("-" * 85)
        for i in mycursor:
    ...
    for x in mycursor:
        ...
        if cnt !=0 :
            ...
            if qty <= pqty:
                ...
                val = (random.randint(0,9999), datetime.datetime.now(), pcode, price, qty, total)
                mycursor.execute(sql, val)
                sql = "UPDATE product SET pqty=pqty-%s WHERE pcode=%s"
                val = (qty, pcode)
                mycursor.execute(sql, val)
